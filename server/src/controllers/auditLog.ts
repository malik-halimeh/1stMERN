...
export const getAuditLogs = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (actorId) {
      if (!mongoose.Types.ObjectId.isValid(actorId as string)) {
        throw new AppError('VALIDATION_FAILED', 'Invalid actor ID format.', 422);
      }
      filter.actorId = new mongoose.Types.ObjectId(actorId as string);
    }
    ...
    if (targetEntityId) {
      if (!mongoose.Types.ObjectId.isValid(targetEntityId as string)) {
        throw new AppError('VALIDATION_FAILED', 'Invalid target entity ID format.', 422);
      }
      filter.targetEntityId = new mongoose.Types.ObjectId(targetEntityId as string);
    }
