...
const getVariantPrice = (basePrice, priceDelta) => basePrice + priceDelta;
...
export const getCart = async (req, res, next) => {
    try {
        ...
        if (cartModified) {
            cart.items = cart.items.filter((item) => validatedItems.some((val) => val.productId.toString() === item.productId.toString() && val.variantSku === item.variantSku));
            await cart.save();
        }
        res.status(200).json({
            success: true,
...
export const addToCart = async (req, res, next) => {
    try {
        ...
        if (variant.stock < qty) {
            throw new AppError('INSUFFICIENT_STOCK', `Insufficient stock. Only ${variant.stock} items are available.`, 409);
        }
        const userId = new mongoose.Types.ObjectId(req.user.userId);
        let cart = await Cart.findOne({ userId });
        if (!cart) {
...
export const updateCartItem = async (req, res, next) => {
    try {
        ...
        const itemIdx = cart.items.findIndex((item) => item.productId.toString() === productId && item.variantSku === variantSku);
        if (itemIdx === -1) {
...
export const removeCartItem = async (req, res, next) => {
    try {
        ...
        cart.items = cart.items.filter((item) => !(item.productId.toString() === productId && item.variantSku === variantSku));
...
export const mergeCart = async (req, res, next) => {
    try {
        ...
        for (const guestItem of items) {
            ...
            const existingIdx = cart.items.findIndex((item) => item.productId.toString() === productId && item.variantSku === variantSku);
            if (existingIdx > -1) {
