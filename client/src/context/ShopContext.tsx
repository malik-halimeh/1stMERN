...
interface ShopContextType {
  ...
  /** Adds to cart via API (auth) — resolves with the new cart count */
  addItemToCart: (productId: string, variantSku: string, quantity?: number) => Promise<number>;
...
const readGuestCartCount = (): number => {
  try {
    ...
    if (!stored) return 0;
    return sumQuantities(JSON.parse(stored).items);
  } catch {
    return 0;
  }
};
...
export const ShopProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addItemToCart = useCallback(
    async (productId: string, variantSku: string, quantity = 1): Promise<number> => {
      // Guests are redirected to login by the calling page (API returns 401)
      const res = await api.post('/cart/items', { productId, variantSku, quantity });
      const newCount = sumQuantities(res.data?.data?.items);
