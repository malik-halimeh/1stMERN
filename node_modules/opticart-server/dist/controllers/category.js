...
export const getCategories = async (req, res, next) => {
    try {
        ...
        const categoryTree = rootCategories.map((root) => {
            const subcategories = allCategories.filter((sub) => sub.parentId?.toString() === root._id.toString());
            return {
                _id: root._id,
                name: root.name,
                slug: root.slug,
...
export const deleteCategory = async (req, res, next) => {
    try {
        ...
        if (hasSubs) {
            throw new AppError('CATEGORY_HAS_SUBCATEGORIES', 'Cannot delete category containing active subcategories. Delete subcategories first.', 409);
        }
